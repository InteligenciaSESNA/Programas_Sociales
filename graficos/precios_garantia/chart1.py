    ...
    def beneficiarios_popup(ent, mun, gmargina, numbenef, monto, tprod):
            ...
            result = html.Div([
                ...
                dbc.Row([
                    ...
                    dmc.Text(['Tamaño productor: ', tprod]),
                    dmc.Space(h=4),
    ...
    def benef_choice(benef_sel):
        ...
        if benef_sel == 'Número de Beneficiarios':
            benef_option = dl.Overlay(dl.LayerGroup([dl.CircleMarker(center=[lat, lon], radius=radio,fillOpacity=1,fillColor=color, color=color, children=[
                ...
                dl.Popup(beneficiarios_popup(ent, mun, gmargina, numbenef, monto, tprod))
                ]) for ent, mun, lat, lon, radio, color, gmargina, numbenef, monto, tprod in zip(benef_filter['entidad'], benef_filter['municipio'], benef_filter['latitud'], benef_filter['longitud'], benef_filter['benef_total_radio'], benef_filter['gm_color'], benef_filter['gm'], benef_filter['benef_total'], benef_filter['monto_total'], benef_filter['tipo'])]), name='Beneficiarios', checked=True)
        else:
