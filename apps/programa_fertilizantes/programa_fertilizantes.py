...
# gráficos
from graficos.programa_fertilizantes.chart1 import mapa1
#from graficos.programa_fertilizantes.chart2 import barplot1
# secciones del visualizador
from apps.programa_fertilizantes import reglas_operacion
from apps.programa_fertilizantes.seccion1 import seccion1
from apps.programa_fertilizantes.seccion2 import seccion2
from apps.programa_fertilizantes.seccion3 import seccion3
from apps.programa_fertilizantes.seccion4 import seccion4
from apps.programa_fertilizantes.seccion5 import seccion5
from apps.programa_fertilizantes.seccion6 import seccion6
# importa modulos de seccion 7
from apps.programa_fertilizantes.seccion7 import get_card_poblacion_indigena
from apps.programa_fertilizantes.seccion7 import get_card_poblacion_beneficiaria_img
from apps.programa_fertilizantes.seccion7 import get_card_poblacion_beneficiaria_texto
from apps.programa_fertilizantes.seccion7 import get_card_poblacion_beneficiaria
from apps.programa_fertilizantes.seccion7 import get_card_poblacion_mujeres
from apps.programa_fertilizantes.seccion7 import get_card_monto_promedio
...
#########      CALL : Regresa año  ################
@app.callback(# 'click_feature
        Output('fert_anio_filtro1', 'children'),
        Output('fert_producto_filtro1', 'children'),
        Input('fert_submit-button', 'n_clicks'),
        State('fert_producto', 'value'),
        State('fert_anio', 'value'),
    )
...
@app.callback(# 'click_feat
        Output('fert_anio_filtro2', 'children'),
        Output('fert_producto_filtro2', 'children'),
        Input('fert_submit-button', 'n_clicks'),
        State('fert_producto', 'value'),
        State('fert_anio', 'value'),
    )
...
@app.callback(
    Output("fert_modal", "is_open"),
    [Input("fert_open", "n_clicks"),
     Input("fert_close", "n_clicks")],
    [State("fert_modal", "is_open")],
)
...
@app.callback(
        Output('fert_reglas-operacion', 'children'),
        Input('fert_submit-button', 'n_clicks'),
        State('fert_producto', 'value'),
        State('fert_anio', 'value'),
    )
...
@app.callback(
    Output("fert_transition-instrucciones", "is_in"),
    [Input("fert_transition-instrucciones-btn", "n_clicks")],
    [State("fert_transition-instrucciones", "is_in")],
...
tab1_capas_criterios = html.Div([
    ...
    dmc.RadioGroup(
            ...
            id="fert_beneficiarios-opciones",
                            ...
                            dmc.Card([
                                dbc.Row([
                                    ...
                                    dbc.Col([
                                        ...
                                        "1,332",
                                        ], id='fert_resumen-poblacion-indigena', style={'marginTop':'0em','text-align': 'center', "color":"red", 'font-size': '32px'}),
                                        )]),
                                        dbc.Row([
                ...
                dmc.Tooltip(
                        ...
                        children=[
                            dmc.Card([
                                dbc.Row([
                                    dbc.Col([ # col-sm-12 col-md-6
                                        html.Img(id='fert_image-poblacion_beneficiaria', src='../assets/poblacionBeneficiaria.png', width="63", height="65"),
                                    ],className="card col-lg-3 border-0 bg-transparent", style={'paddingTop':'0rem', 'paddingBottom':'0rem', 'marginTop':'0rem', 'marginBottom':'0rem', 'textAlign': 'left'}),
                                    dbc.Col([
                                        dbc.Row([html.Center(html.Div([
                                        "1,332",
                                        ], id='fert_resumen-poblacion_beneficiaria', style={'marginTop':'0em','text-align': 'center', "color":"blue", 'font-size': '32px'}),
                                        )]),
                                        dbc.Row([
                                            dmc.Text("Población Beneficiaria", id="fert_resumen_texto_poblacion_beneficiaria" , color='grey', weight=500, align='center', style={"fontSize": 10})
                                            ]),
                                    ], className="card col-9 border-0 bg-transparent"),
                                ], style={'border-radius': '5px', 'padding':'0rem', 'paddingLeft':'0.2rem'}),
                            ],
                            ...
                            dmc.Card([
                                dbc.Row([
                                    ...
                                    dbc.Col([
                                        dbc.Row([html.Center(html.Div([
                                        "51%",
                                        ], id='fert_resumen-poblacion-mujeres', style={'padding':'0em',"textAling":"right", "color":"grey", 'font-size': '32px'}),
                                        )]),
                                        dbc.Row([
                            ...
                            dmc.Card([
                                dbc.Row([
                                    ...
                                    dbc.Col([
                                        dbc.Row([html.Center(html.Div([
                                        "1,332",
                                        ], id='fert_resumen-monto-promedio', style={'paddingRight':'0rem','text-align': 'center', "color":"green", 'font-size': '32px'}),
                                        )]),
                                        dbc.Row([
...
@app.callback(Output("fert_content-capas-criterios", "children"),
              Output("fert_mapa", "children"),
             [Input("fert_capas-criterios", "value")],
             #prevent_initial_call=True,
            )
            
def switch_tab(active):
    if active == "capas": # dmc.Loader(color="red", size="md", variant="oval")
        return tab1_capas_criterios,  dcc.Loading(id="fert_ls-loading-1",children=content_mapa1, type="default")
    elif active == "criterios":
        return tab2_capas_criterios, dcc.Loading(id="fert_ls-loading-2",children=content_mapa2, type="default") #content_mapa2
    return html.P("This shouldn't ever be displayed...")

#########  CALL : Regresa actualización del MAPA  ################
# declaración de parámetros para color y leyendas

@app.callback(Output("fert_ls-loading-output-1", "children"), 
              Input("fert_ls-input-1", "value"),
              #prevent_initial_call=True,
   )
...
@app.callback(Output("fert_ls-loading-output-2", "children"), 
              Input("fert_ls-input-2", "value"),
             # prevent_initial_call=True,
    )
...
@app.callback(# 'click_feature
        Output('fert_state_label', 'children'),
        Input("fert_states", "click_feature"),
        #prevent_initial_call=True,
    )
def get_state(clicks, feature):
    ...
    if not feature:
        return [
            ...
            dmc.Center(html.Img(id='fert_image', src='../assets/'+ str("Nacional") +'.png', width="65", height="65")),
          ]
    else:
        # filtro de estado
        state = feature["properties"]["name"]
        ...
        return [
            html.H4("{}".format(feature["properties"]["name"])),
            dmc.Center(html.Img(id='fert_image', src='../assets/'+ str(feature["properties"]["name"]) +'.png', width="65", height="65")),
          ]

# actualiza infor en mapa
@app.callback(Output("fert_info", "children"),
              Input("fert_states", "click_feature"),
        )
...
content_mapa1 = html.Div(dl.Map(center=[22.76, -102.58], zoom=5,
             id="fert_mapa1", attributionControl=False,  style={'width': '100%', 'height': '100vh', 'backgroundColor':'white', 'margin': "auto", "display": "block"}),
)


content_mapa2 = html.Div(dl.Map(center=[22.76, -102.58], zoom=5,
             id="fert_mapa2", attributionControl=False,  style={'width': '100%', 'height': '100vh', 'backgroundColor':'white', 'margin': "auto", "display": "block"}),
)


##   CALLBACK : MAPA
@app.callback(
        Output('fert_mapa1', 'children'),
        Input('fert_submit-button', 'n_clicks'),
        Input("fert_beneficiarios-opciones", "value"),
        Input("fert_transfer-list-simple", "value"),
        State('fert_producto', 'value'),
        State('fert_anio', 'value'),
    )
...
capa_base = dl.Pane(dl.GeoJSON(data=data2,  # url to geojson file
                        ...
                        id='fert_states'), name="base2", style={'zIndex':0}) 
    
######### Mapa criterios simulados   ################
##   CALLBACK : MAPA
@app.callback(  
        Output('fert_mapa2', 'children'),
        Input('fert_submit-button', 'n_clicks'),
        Input('fert_criterios1', 'value'),
        State('fert_producto', 'value'),
        State('fert_anio', 'value'),
    )
